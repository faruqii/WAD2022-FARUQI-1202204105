...
if (isset($_POST["daftar"])) {
    ...
    $password = mysqli_real_escape_string($connuser, $_POST["password"]);
    $pwdConfirm = mysqli_real_escape_string($connuser, $_POST["password2"]);
    $no_hp = $_POST["no"];
    $result = mysqli_query($connuser, "SELECT email FROM user_faruqi WHERE email = '$email'");

    if (mysqli_num_rows(($result)) !== 1 && $password == $pwdConfirm) {
        $password = password_hash($password, PASSWORD_DEFAULT);
        mysqli_query($connuser, "INSERT INTO user_faruqi VALUES ('', '$nama', '$email', '$password', '$no_hp')");
